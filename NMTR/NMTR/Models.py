        ...
        with tf.name_scope("embedding"):
            ...
            self.h = tf.Variable(
                tf.random_uniform([h_size, 1], minval=-tf.sqrt(3 / h_size),
                                  maxval=tf.sqrt(3 / h_size)), name='h')
    ...
    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            ...
            if self.layer_num == 0:
                ...
                return tf.sigmoid(tf.matmul(fc[-1], self.h), name = 'output')

    def _create_loss(self):
        with tf.name_scope("loss"):        
            self.output = self._create_inference(self.item_input)
            self.em_reg_loss = self.embedding_reg * (tf.reduce_sum(tf.square(self.embedding_P))+tf.reduce_sum(tf.square(self.embedding_Q)))

            loss_W_FC = 0
            if self.layer_num == 1:
                loss_W_FC = self.fc_reg * tf.reduce_sum(tf.square(self.W_FC))
            else:
                for i in range(len(self.W_FC)):
                    loss_W_FC += self.fc_reg * tf.reduce_sum(tf.square(self.W_FC[i]))

            self.fc_reg_loss = loss_W_FC
            self.loss = tf.losses.log_loss(self.labels, self.output) + self.em_reg_loss + self.fc_reg_loss
...
class pure_NCF:
    ...
    def _create_loss(self):
        with tf.name_scope("loss"):        
            ...
            self.fc_reg_loss = loss_W_FC
            self.loss = tf.losses.log_loss(self.labels, self.output) + self.em_reg_loss + self.fc_reg_loss
            self.loss_buy = tf.losses.log_loss(self.labels, self.output)
...
class Multi_GMF():
    def __init__(self, num_users, num_items, args):
        ...
        self.cascade_mode = args.cascade_mode
    ...
    def _create_loss(self):
        with tf.name_scope('loss'):
            if self.loss_func == 'logloss':
                ...
                self.loss_reg_1 = self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_P)) + \
                                  self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_Q)) + \
                                  self.fc_reg * tf.reduce_sum(tf.square(self.W1))

                self.loss_reg_2 = self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_P)) + \
                                  self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_Q)) + \
                                  self.fc_reg * tf.reduce_sum(tf.square(self.W1)) + \
                                  self.fc_reg * tf.reduce_sum(tf.square(self.W2)) 

                self.loss_reg_3 = self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_P)) + \
                                  self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_Q)) + \
                                  self.fc_reg * tf.reduce_sum(tf.square(self.W1)) + \
                                  self.fc_reg * tf.reduce_sum(tf.square(self.W2)) + \
                                  self.fc_reg * tf.reduce_sum(tf.square(self.W3)) 
                ...
                if self.b_num == 3:
                    ...
                    if self.cascade_mode == 'pretrain':
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = self.loss_ipv + self.loss_cart + self.loss_reg_2
                        self.loss3 = self.loss_ipv + self.loss_cart + self.loss_buy + self.loss_reg_3                     

                    elif self.cascade_mode == 'frozen':
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = 1/2 * (self.loss_ipv + self.loss_cart) + self.loss_reg_2 - self.loss_reg_1
                        self.loss3 = 1/3 * (self.loss_ipv + self.loss_cart + self.loss_buy) + self.loss_reg_3 - self.loss_reg_2                        

                    else:
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = self.loss_cart + self.loss_reg_2 - self.loss_reg_1
                        self.loss3 = self.loss_buy + self.loss_reg_3 - self.loss_reg_2                        
                    ...
                    if self.b_2_type == 'cb':
                        ...
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = self.loss_buy + self.loss_reg_2 - self.loss_reg_1
...
class Multi_MLP():
    ...
    def _create_variables(self):
        ...
        with tf.name_scope('ipv_para'):   
            h_size = 2 * int(self.embedding_size/(2**self.layer_num))
            self.h_1 = tf.Variable(tf.random_uniform([h_size, 1], minval=-tf.sqrt(3 / h_size),
        ...
        with tf.name_scope('cart_para'):   
            h_size = 2 * int(self.embedding_size/(2**self.layer_num))
            self.h_2 = tf.Variable(tf.random_uniform([h_size, 1], minval=-tf.sqrt(3 / h_size),
        ...
        with tf.name_scope('buy_para'):   
            h_size = 2 * int(self.embedding_size/(2**self.layer_num))
            self.h_3 = tf.Variable(tf.random_uniform([h_size, 1], minval=-tf.sqrt(3 / h_size),
...
class FISM:
        ...
        with tf.name_scope('inference'):
            for i in range(self.max_rate):
                ...
                if i == 0:
                    ...
                    p_j_sum = p_j_sum + p_j_e  # [B, E]
     
            q_i_e = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q, self.item_input), 1)  # [B, E]
            item_sim = tf.reshape(tf.reduce_sum(q_i_e * p_j_sum, axis=1), shape=[-1, 1])  # [B, 1] dot product
    ...
    def _create_loss(self):
        with tf.name_scope('loss'):
            ...
            self.loss = tf.reduce_sum(tf.pow((self.output - self.labels), tf.constant(2, tf.float32))) + \
                        self.lambda_bilinear * tf.reduce_sum(tf.square(self.embedding_P)) + \
                        self.gamma_bilinear * tf.reduce_sum(tf.square(self.embedding_Q))               
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            self.embedding_P_1 = tf.Variable(
                tf.truncated_normal(shape=[self.num_users, self.embedding_size], mean=0.0, stddev=0.01),
                name='embedding_P_1', dtype=tf.float32)  # (users, embedding_size)
            self.embedding_Q_1 = tf.Variable(
                tf.truncated_normal(shape=[self.num_items, self.embedding_size], mean=0.0, stddev=0.01),
                name='embedding_Q_1', dtype=tf.float32)  # (items, embedding_size)
            self.embedding_P_2 = tf.Variable(
                tf.truncated_normal(shape=[self.num_users, self.embedding_size], mean=0.0, stddev=0.01),
                name='embedding_P_2', dtype=tf.float32)  # (users, embedding_size)
            self.embedding_Q_2 = tf.Variable(
                tf.truncated_normal(shape=[self.num_items, self.embedding_size], mean=0.0, stddev=0.01),
                name='embedding_Q_2', dtype=tf.float32)  # (items, embedding_size)            
        ...
        with tf.name_scope("FC"):         
            ...
            if self.layer_num == 0:
                ...
                self.W_FC = tf.Variable(tf.random_uniform(shape=[2*self.embedding_size, int(self.embedding_size)],
                                                    minval=-tf.sqrt(3 / (2*self.embedding_size)),
                                                    maxval=tf.sqrt(3 / (2*self.embedding_size))), name='W_FC')
                self.b_FC = tf.Variable(tf.zeros([1, self.embedding_size]), dtype=tf.float32, name='b_FC')
            else:
        ...
        with tf.name_scope("Neu_MF"): 
            h_len = self.embedding_size + output_size
            self.h = tf.Variable(tf.random_uniform([h_len, 1], 
                                 minval=-tf.sqrt(3 / h_len),
                                 maxval=tf.sqrt(3 / h_len)), name='h')


    def _create_inference(self):
        with tf.name_scope("inference"):
            self.embedding_p_1 = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_P_1, self.user_input), 1)
            self.embedding_q_1 = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_1, self.item_input), 1)
            self.embedding_p_2 = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_P_2, self.user_input), 1)
            self.embedding_q_2 = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_2, self.item_input), 1)   

            self.gmf_vec = self.embedding_p_1 * self.embedding_q_1      
            self.concat_vec = tf.concat([self.embedding_p_2, self.embedding_q_2], 1, name='concat_vec')
            if self.layer_num == 0:
                return tf.sigmoid(tf.matmul(self.concat_vec, self.h), name = 'output')  # (b, embedding_size) * (embedding_size, 1)

            elif self.layer_num == 1:
                fc = tf.nn.relu(tf.matmul(self.concat_vec, self.W_FC) + self.b_FC)
                return tf.sigmoid(tf.matmul(tf.concat([fc, self.gmf_vec], 1), self.h), name = 'output')
                ...
                return tf.sigmoid(tf.matmul(tf.concat([fc[i], self.gmf_vec], 1), self.h), name = 'output')


    def _create_loss(self):
        with tf.name_scope("loss"):        
            self.output = self._create_inference()
            self.em_reg_loss = self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_P_1)) + \
                               self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_Q_1)) + \
                               self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_P_2)) + \
                               self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_Q_2))
...
class Multi_NCF_2():
    ...
    def _create_variables(self):
        ...
        with tf.name_scope('ipv_para'):
            # gmf specific
            self.W10 = tf.Variable(
                tf.random_uniform(shape = [self.embedding_size, self.embedding_size],
                    minval = -tf.sqrt(3 / self.embedding_size), maxval = tf.sqrt(3 / self.embedding_size),
                    name = 'W10', dtype = tf.float32))
            ...
            # merge
            h_size = self.embedding_size + output_size
            self.h_1 = tf.Variable(tf.random_uniform([h_size, 1], minval=-tf.sqrt(3 / h_size),
                                   maxval=tf.sqrt(3 / h_size)), name='h_1')
        ...
        with tf.name_scope('cart_para'):
            # gmf specific
            self.W20 = tf.Variable(
                tf.random_uniform(shape = [self.embedding_size, self.embedding_size],
                    minval = -tf.sqrt(3 / self.embedding_size), maxval = tf.sqrt(3 / self.embedding_size),
                    name = 'W20', dtype = tf.float32))
        ...
        with tf.name_scope('buy_para'):
            # gmf specific
            self.W30 = tf.Variable(
                tf.random_uniform(shape = [self.embedding_size, self.embedding_size],
                    minval = -tf.sqrt(3 / self.embedding_size), maxval = tf.sqrt(3 / self.embedding_size),
                    name = 'W30', dtype = tf.float32))
    ...
    def _create_inference(self):
        ...
        with tf.name_scope('inference'):
            ...
            embedding_q_1 = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_1, self.item_input), 1)
            embedding_p_2 = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_P_2, self.user_input), 1)
            embedding_q_2 = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_2, self.item_input), 1)   

            self.gmf_vec_1 = tf.nn.relu(tf.matmul(embedding_p_1 * embedding_q_1, self.W10))
            self.gmf_vec_2 = tf.nn.relu(tf.matmul(embedding_p_1 * embedding_q_1, self.W20))
            self.gmf_vec_3 = tf.nn.relu(tf.matmul(embedding_p_1 * embedding_q_1, self.W30))

            # [B, 2E]
            z =  tf.concat([embedding_p_2, embedding_q_2], 1, name='z')
            ...
            if self.layer_num == 0:
                ...
                if self.b_num == 3:
                    # predict ipv
                    output_ipv = tf.matmul(tf.concat([tf.nn.relu(tf.matmul(z, self.W1) + self.b1), self.gmf_vec_1], 1), self.h_1) + b
                    # predict cart
                    temp_cart = tf.matmul(tf.concat([tf.nn.relu(tf.matmul(z, self.W2) + self.b2), self.gmf_vec_2], 1), self.h_2)
                    output_cart = (temp_cart + output_ipv) / 2 + b
                    # predict buy
                    temp_buy = tf.matmul(tf.concat([tf.nn.relu(tf.matmul(z, self.W3) + self.b3), self.gmf_vec_3]), self.h_3)
                    output_buy = (temp_buy + output_cart) / 2 + b
                    ...
                    if self.b_2_type == 'cb':
                        # predict cart
                        output_cart = tf.matmul(tf.concat([tf.nn.relu(tf.matmul(z, self.W1) + self.b1), self.gmf_vec_1], 1), self.h_1) + b
                        # predict buy
                        temp_buy = tf.matmul(tf.concat([tf.nn.relu(tf.matmul(z, self.W2) + self.b2), self.gmf_vec_2], 1), self.h_2)
                        output_buy = (temp_buy + output_cart) / 2 + b
                ...
                if self.b_num == 3:
                    # predict ipv
                    output_ipv = tf.matmul(tf.concat([fc_1[i], self.gmf_vec_1], 1), self.h_1) + b
                    # predict cart
                    temp_cart = tf.matmul(tf.concat([fc_2[i], self.gmf_vec_2], 1), self.h_2)
                    output_cart = (temp_cart + output_ipv) / 2 + b
                    # predict buy
                    temp_buy = tf.matmul(tf.concat([fc_3[i], self.gmf_vec_3], 1), self.h_3)
                    output_buy = (temp_buy + output_cart) / 2 + b
                    ...
                    if self.b_2_type == 'cb':
                        # predict cart
                        output_cart = tf.matmul(tf.concat([fc_1[i], self.gmf_vec_1], 1), self.h_1) + b
                        # predict buy
                        temp_buy = tf.matmul(tf.concat([fc_2[i], self.gmf_vec_2], 1), self.h_2)
                        output_buy = (temp_buy + output_cart) / 2 + b
    ...
    def _create_loss(self):
        with tf.name_scope('loss'):
            if self.loss_func == 'logloss':
                loss_em = self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_P_1)) + \
                          self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_Q_1)) + \
                          self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_P_2)) + \
                          self.embedding_reg * tf.reduce_sum(tf.square(self.embedding_Q_2))
                ...
                loss_W0 = self.fc_reg * (tf.reduce_sum(tf.square(self.W10)) +\
                                         tf.reduce_sum(tf.square(self.W20)) +\
                                         tf.reduce_sum(tf.square(self.W30)))
                
                self.loss_reg = loss_em + loss_W1 + loss_W2 + loss_W3 + loss_W0
                self.loss_reg_1 = loss_em + loss_W1 + self.fc_reg * tf.reduce_sum(tf.square(self.W10))
                self.loss_reg_2 = loss_em + loss_W2 + self.fc_reg * tf.reduce_sum(tf.square(self.W20))
                self.loss_reg_3 = loss_em + loss_W3 + self.fc_reg * tf.reduce_sum(tf.square(self.W30))
                ...
                if self.b_num == 3:
                    ...
                    if self.cascade_mode == 'pretrain':
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = self.loss_ipv + self.loss_cart + self.loss_reg_1 + self.loss_reg_2
                        self.loss3 = self.loss_ipv + self.loss_cart + self.loss_buy + self.loss_reg_1 + self.loss_reg_2 + self.loss_reg_3                        


                    elif self.cascade_mode == 'frozen':
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = 1/2 * (self.loss_ipv + self.loss_cart) + self.loss_reg_2
                        self.loss3 = 1/3 * (self.loss_ipv + self.loss_cart + self.loss_buy) + self.loss_reg_3    

                    else:
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = self.loss_cart + self.loss_reg_2
                        self.loss3 = self.loss_buy + self.loss_reg_3    
                    ...
                    if self.b_2_type == 'cb':
                        ...
                        self.loss1 = self.loss_ipv + self.loss_reg_1
                        self.loss2 = self.loss_buy + self.loss_reg_2
...
class CMF:
    ...
    def _create_placeholders(self):
        with tf.name_scope("input_data"):
            self.user_input = tf.placeholder(tf.int32, shape=[None, 1], name = "user_input")
            self.item_input_buy = tf.placeholder(tf.int32, shape=[None, None], name = "item_input_buy")
            self.item_input_cart = tf.placeholder(tf.int32, shape = [None, None], name = 'item_input_cart')
            self.item_input_ipv = tf.placeholder(tf.int32, shape = [None, None], name = 'item_input_ipv')
            self.labels_buy = tf.placeholder(tf.float32, shape=[None, 1], name="labels_buy")  # (b,1)
            self.labels_cart = tf.placeholder(tf.float32, shape=[None, 1], name="labels_cart")  # (b,1)
            self.labels_ipv = tf.placeholder(tf.float32, shape=[None, 1], name="labels_ipv")  # (b,1)
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.embedding_Q_buy = tf.Variable(
                tf.truncated_normal(shape=[self.num_items, self.embedding_size], 
                    mean=0.0, stddev=0.01), name='embedding_Q_buy', dtype=tf.float32)  #(items, embedding_size)

            self.embedding_Q_cart = tf.Variable(
                tf.truncated_normal(shape=[self.num_items, self.embedding_size], 
                    mean=0.0, stddev=0.01), name='embedding_Q_cart', dtype=tf.float32)  #(items, embedding_size)
            
            self.embedding_Q_ipv = tf.Variable(
                tf.truncated_normal(shape=[self.num_items, self.embedding_size], 
                    mean=0.0, stddev=0.01), name='embedding_Q_ipv', dtype=tf.float32)  #(items, embedding_size)
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            self.embedding_p = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_P, self.user_input), 1)
            self.embedding_q_buy = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_buy, self.item_input_buy), 1)  # (b, embedding_size)
            self.embedding_q_cart = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_cart, self.item_input_cart), 1)  # (b, embedding_size)
            self.embedding_q_ipv = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_ipv, self.item_input_ipv), 1)  # (b, embedding_size)
            
            output_buy = tf.sigmoid(
                tf.matmul(self.embedding_p * self.embedding_q_buy, self.h), name = 'output_buy')  # (b, embedding_size) * (embedding_size, 1)

            output_cart = tf.sigmoid(
                tf.matmul(self.embedding_p * self.embedding_q_cart, self.h), name = 'output_cart')

            output_ipv = tf.sigmoid(
                tf.matmul(self.embedding_p * self.embedding_q_ipv, self.h), name = 'output_ipv')

            return (output_buy, output_cart, output_ipv)

    def _create_loss(self):
        with tf.name_scope("loss"):
            if self.loss_func == "logloss":
                
                self.output_buy, self.output_cart, self.output_ipv = self._create_inference()

                self.loss_no_reg = tf.losses.log_loss(self.labels_buy, self.output_buy) + \
                    tf.losses.log_loss(self.labels_cart, self.output_cart) + \
                    tf.losses.log_loss(self.labels_ipv, self.output_ipv)

                self.loss_reg = self.lambda_bilinear * tf.reduce_sum(tf.square(self.embedding_P)) + \
                                self.gamma_bilinear * tf.reduce_sum(tf.square(self.embedding_Q_buy)) + \
                                self.gamma_bilinear * tf.reduce_sum(tf.square(self.embedding_Q_cart)) + \
                                self.gamma_bilinear * tf.reduce_sum(tf.square(self.embedding_Q_ipv))

                self.loss = self.loss_no_reg + self.loss_reg
