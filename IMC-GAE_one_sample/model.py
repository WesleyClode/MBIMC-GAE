...
class MLPDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.lin1 = nn.Linear(in_units, 128)
    ...
    def forward(self, graph, ufeat, ifeat):
        ...
        with graph.local_scope():
            ...
            out = graph.edata['m']
            print("MLP out shape1:",out.shape)
            #out = th.tanh(out)
            ...
            out = self.lin2(out)
            out = F.relu(out)
            print("MLP out shape2:",out.shape)
        return out
    ...
    def forward(self, graph, ufeat, ifeat):
        ...
        with graph.local_scope():
            ...
            for i in range(self._num_basis):
                ...
                #print("self.Ps[", i ,"]:", self.Ps[i].shape)
                #print("graph.nodes['user'].data['h']:", graph.nodes['user'].data['h'].shape)
                #print("ufeat:", ufeat.shape)
                graph.apply_edges(fn.u_dot_v('h', 'h', 'sr'))
                #print("fn.u_dot_v('h', 'h', 'sr'):",fn.u_dot_v('h', 'h', 'sr'))
                basis_out.append(graph.edata['sr'])
                #print("graph.edata['sr']:", graph.edata['sr'].shape)
            out = th.cat(basis_out, dim=1)
            #print("basis_out:",len(basis_out),len(basis_out[0]))
            #print("out:",out.shape)
