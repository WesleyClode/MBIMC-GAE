...
from model import BiDecoder, GCMCLayer, MLPDecoder, DenseBiDecoder
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        if args.decoder == "Bi":
            ...
            self.decoder = DenseBiDecoder(in_units= args.gcn_out_units, #* args.layers,
                                     num_classes=len(args.rating_vals),
                                     num_basis=args.gen_r_num_basis_func)
...
class Multi_lalel_loss(nn.Module):
    def __init__(self):
        super().__init__()
        
    def forward(self, x, y):
        return -th.sum(x * y, dim=(0, 1))
...
def train(args):
    ...
    Multi_lalel_net = Multi_lalel_loss()
    ...
    def update_encode_graph(dataset, train_rating_pairs, train_rating_values, sampled_data):
        ...
        dataset.train_dec_graph = dataset._generate_dec_graph(train_rating_pairs)
        dataset.train_enc_graph = dataset.train_enc_graph.int().to(args.device)
        dataset.train_dec_graph = dataset.train_dec_graph.int().to(args.device)
        #dataset.valid_enc_graph = dataset.train_enc_graph    
        return dataset.train_enc_graph, dataset.train_dec_graph, train_rating_values
    ...
    # negitive 为每个user对应的所有的负样本点
    negitive_all = dataset.negative_all(dataset.train_rating_info)

    # sampled_data为从negitive_all中抽取sample_rate = 40个样本的数据
    sampled_data = sample_data(negitive_all, random_number = 1, sample_rate = 40)
    
    # 构建新的含有负样本的enc_graph，dec_graph，以及每条边对应的rating
    dataset.train_enc_graph, dataset.train_dec_graph, train_rating_values = update_encode_graph(dataset, train_rating_pairs, train_rating_values, sampled_data)
    dataset.valid_enc_graph = dataset.train_enc_graph 

    # prepare ground truth 数据，rating变成onehot，最后用于计算CE loss
    train_gt_ratings = train_rating_values
    train_gt_ratings_new = np.zeros((train_gt_ratings.shape[0], 2))
    for i in range(train_gt_ratings.shape[0]):
        k = int(train_gt_ratings[i])
        #print(k)
        train_gt_ratings_new[i][k] = 1

    train_gt_ratings_new = th.FloatTensor(train_gt_ratings_new).to(args.device)
    ...
    for iter_idx in range(1, args.train_max_iter):
        ...
        print("pred:",pred_ratings[0:10])
        #sigmoid = nn.Sigmoid()
        rating_loss_net1 = nn.BCELoss()
        rating_loss_net2 = nn.BCELoss()
        rating_loss_net3 = nn.BCELoss()
        rating_loss_net4 = nn.BCELoss()

        print("pred shape:", pred_ratings[:,0].shape)
        #print("train_gt_rating_new:",train_gt_ratings_new[0:10].shape)
        loss1 = rating_loss_net1(th.sigmoid(pred_ratings[:,0]), train_gt_ratings_new[:,0])
        loss2 = rating_loss_net2(th.sigmoid(pred_ratings[:,1]), train_gt_ratings_new[:,1])

        #print("train_gt_ratings_new:",train_gt_ratings_new[-10:-1])
        #loss3 = rating_loss_net3(th.sigmoid(pred_ratings[:,2]), train_gt_ratings_new[:,2])
        #loss4 = rating_loss_net4(th.sigmoid(pred_ratings[:,3]), train_gt_ratings_new[:,3])
...
def config():
    ...
    parser.add_argument('--gcn_agg_units', type=int, default=64)
    parser.add_argument('--gcn_agg_accum', type=str, default="sum")
    parser.add_argument('--gcn_out_units', type=int, default=75)
    parser.add_argument('--gen_r_num_basis_func', type=int, default=5)
