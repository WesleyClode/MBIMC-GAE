...
class DataSetLoader(object):
    ...
    def __init__(self, name, device, mix_cpu_gpu=False,
        ...
        elif self._name in ['flixster', 'douban', 'yahoo_music','Tmall','Tmall_small'] or self._name[0:5] == 'Tmall':
        ...
        train_rating_pairs_zeros, train_rating_values_zeros = self._generate_pair_value_for_zero(self.train_rating_info, self.negatives)

        print("train_rating_pairs1:", len(train_rating_pairs[0]))

        train_rating_pairs = (np.append(train_rating_pairs[0], train_rating_pairs_zeros[0]),\
             np.append(train_rating_pairs[1], train_rating_pairs_zeros[1]))

        print("train_rating_pairs2:", len(train_rating_pairs[0]))

        train_rating_values = np.append(train_rating_values, train_rating_values_zeros)

        print("train_rating_values:", len(train_rating_values))
        ...
        #print("self.train_enc_graph:", self.train_enc_graph.values)
        #print("self.train_enc_graph:", self.train_enc_graph.values)
        ...
        test_recall_pair, test_rating_matrix = self._generate_pair_value_for_recall(self.test_rating_info)
        ...
        print("Valid recall dec graph: \t#user:{}\t#movie:{}\t#pairs:{}".format(
            self.valid_recall_dec_graph.number_of_nodes('user'), self.valid_recall_dec_graph.number_of_nodes('movie'),
            self.valid_recall_dec_graph.number_of_edges()))
        ...
        print("Test recall dec graph: \t#user:{}\t#movie:{}\t#pairs:{}".format(
            self.test_recall_dec_graph.number_of_nodes('user'), self.test_recall_dec_graph.number_of_nodes('movie'),
            self.test_recall_dec_graph.number_of_edges()))
            
    def sample_negative(self, ratings, sample_rate):
    ...
    def _generate_dec_graph(self, rating_pairs):
        ...
        print("ratings_coo:",rating_pairs)
        print("ratings_coo:",len(rating_pairs[0]))
        print("ratings_coo:",len(rating_pairs[1]))
        print("self.num_user, self.num_movie:",self.num_user, self.num_movie)
    ...
    def _load_raw_rates_Tmall(self, file_path, sep):
        ...
        rating_info = pd.read_csv(
            file_path, sep=sep, header=None,
            names=['user_id', 'movie_id', 'rating', 'timestamp'],
            dtype={'user_id': np.int64, 'movie_id' : np.int64,
                   'ratings': np.float32, 'timestamp': np.int64}, engine='python',encoding="ISO-8859-1")
        return rating_info
