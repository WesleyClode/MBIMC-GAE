...
class DataSetLoader(object):
    def __init__(self, name, device, mix_cpu_gpu=False,
        ...
        if self._name in ['ml-100k', 'ml-1m', 'ml-10m']:
            ...
            tp_train.insert(tp_train.shape[1], 'rating', 3)
            tp_test.insert(tp_test.shape[1], 'rating', 3)
            tp_view = tp_view.drop(columns='3')
            tp_view.insert(tp_view.shape[1], 'rating', 1)
            tp_cart = tp_cart.drop(columns='3')
        ...
        elif self._name in ['flixster', 'douban', 'yahoo_music','Tmall','Tmall_small','taobao'] or self._name[0:5] == 'Tmall' or self._name[0:6] == 'taobao':
